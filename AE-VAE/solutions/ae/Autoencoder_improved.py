...
class Decoder_improved(nn.Module):
    """Same decoder as above, with a sigmoid on the output layer.

    This is the whole fix: the images live in [0, 1], so the last layer of the decoder
    must too. Without it the network is asked to reproduce bounded values with an
    unbounded output, and the reconstruction stays blurred.
    """
...
class Autoencoder_improved(nn.Module):
    """Two-layer encoder and decoder, the decoder ending with a sigmoid."""
...
def train_autoencoder(autoencoder,
    ...
    """Train a dense autoencoder and return it along with the last train and validation losses."""
